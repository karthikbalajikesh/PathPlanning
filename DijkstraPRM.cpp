...
DijkstraPRM::DijkstraPRM(pair<double, double> destination, int numIterations, int zmax, int xmax, int cellWidth, int cellHeight, double radius, cood_vector coordinates) :
...
void DijkstraPRM::addDestination(pair<double, double>& dest) {
...
vector<GraphNode> DijkstraPRM::shortestPath() {
	...
	vector<double> distance_from_source(network.numVertices, INT_MAX);
...
void DijkstraPRM::relaxNode(Queue& minHeap, int& currentIndex, double& currentDistance,
